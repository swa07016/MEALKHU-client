...
const FeedbackPage = ({ match }) => {
  ...
  const setFeedbackDescription = () => {
    if (match.params.name === 'seoul')
      return '서울캠 식당들의 정보수정이나 식당추가 등의 피드백을 남겨주세요!';
    else if (match.params.name === 'global')
      return '국제캠 식당들의 정보수정이나 식당추가 등의 피드백을 남겨주세요!';
    else return '기타 개선사항을 자유롭게 이야기해 주세요!';
  };

  const setfullUrl = () => {
    if (process.env.NODE_ENV === 'production')
      return `https://www.mealkhu.com/feedback/${match.params.name}`;
      ...
      >
        ...
        <div>
          ...
          <Button
            ...
            className="rounded-pill mx-2 my-1 remove-hover"
            style={
              match.params.name === 'global' ? styles.onBtn : styles.offBtn
            }
            value="global"
